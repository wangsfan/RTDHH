...
def cdist(X, Y, *, c=1.0, keepdim=False): # 功能应该同dist_matrix/dist_matrix_A？只是cdist逐对儿计算distance，后者则以矩阵形式算所有对儿的distances
    """
    Compute the pairwise distances between two batches of points on the Poincare ball.
    like torch.cdist

    Parameters
    ----------
    X : tensor
        Batch of points on the Poincare ball with shape (B, M, D) or (M, D)
    Y : tensor
        Batch of points on the Poincare ball with shape (B, N, D) or (N, D)
    c : float|tensor
        Ball negative curvature
    keepdim : bool
        Retain the last dim? (default: false)
    ...
    -------
    tensor
        Pairwise distances between each point in X and each point in Y with shape (M, P) if B=1, otherwise (B, M, P)
    """
    c = torch.as_tensor(c).type_as(X)

    # Determine the shape of X and Y
    if X.dim() == 2:
        X = X.unsqueeze(0)  # Shape: (1, M, D)
    if Y.dim() == 2:
    ...
    # Reshape X and Y to compute pairwise distances
    X_expanded = X.unsqueeze(2).expand(B, M, N, -1)  # Shape: (B, M, N, D)
    Y_expanded = Y.unsqueeze(1).expand(B, M, N, -1)  # Shape: (B, M, N, D)

    # Compute the distance for each pair using vectorized operations
    distances = dist(X_expanded, Y_expanded, c=c, keepdim=keepdim)

    # If B=1, squeeze the batch dimension
    if B == 1:
        distances = distances.squeeze(0)  # Shape: (M, N)
...
def _mobius_addition_batch(x, y, c): # x: B x D, y: C x D
    xy = _tensor_dot(x, y)  # B x C
    x2 = x.pow(2).sum(-1, keepdim=True)  # B x 1
    y2 = y.pow(2).sum(-1, keepdim=True)  # C x 1
    num = 1 + 2 * c * xy + c * y2.permute(1, 0)  # B x C
    num = num.unsqueeze(2) * x.unsqueeze(1) # B x C x D
    num = num + (1 - c * x2).unsqueeze(2) * y  # B x C x D
    denom_part1 = 1 + 2 * c * xy  # B x C
    denom_part2 = c ** 2 * x2 * y2.permute(1, 0) # B x C
    denom = denom_part1 + denom_part2 # B x C
...
def dist_matrix(x, y, c=1.0): # 注！？ B=C时、即x与y形状相同时，用dist直接计算不会报错，但是结果的形状为B，而非B*B，可能此时用dist计算的是xy对应行向量的距离，而非遍历x，y各自所有行向量的距离
    c = torch.as_tensor(c).type_as(x)
    return _dist_matrix(x, y, c) # B x C

def _mobius_addition_A_batch(x, y, c): # x: A x B x D, y: A x C x D
    xy = torch.einsum("abd,acd->abc", (x, y))  # A x B x C
    x2 = x.pow(2).sum(-1, keepdim=True)  # A x B x 1
    y2 = y.pow(2).sum(-1, keepdim=True)  # A x C x 1
    num = 1 + 2 * c * xy + c * y2.permute(0, 2, 1)  # A x B x C
    num = num.unsqueeze(3) * x.unsqueeze(2)  # A x B x C x D
    num = num + (1 - c * x2).unsqueeze(3) * y.unsqueeze(1)  # A x B x C x D
    denom_part1 = 1 + 2 * c * xy  # A x B x C
    denom_part2 = c ** 2 * x2 * y2.permute(0, 2, 1)  # A x B x C
    denom = denom_part1 + denom_part2  # A x B x C
    res = num / (denom.unsqueeze(3) + 1e-5)  # A x B x C x D
    return res

def dist_matrix_A(x, y, c=1.0):
    c = torch.as_tensor(c).type_as(x)
    sqrt_c = c ** 0.5
    return (
            2 / sqrt_c
            * artanh(sqrt_c * torch.norm(_mobius_addition_A_batch(-x, y, c=c), dim=-1))
    ) # A x B x C
