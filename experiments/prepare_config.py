...
def prepare_config(args):
    ...
    if config['method'] == "RTDHH":
        if config['ada_ft_weight']:
            config['id'] += "_ada_fw"
        else:
            config['id'] += f"_fw{config['ft_weight']}"
        config['id'] += f"_bw{config['bt_weight']}"
        config['id'] += f"_C_bdr{config['C_norm_bdr']}"
    ...
    config['logging_path_root'] = os.path.join(config['group'], config['logging_path_root'])
