...
def get_lifelong_datasets(dataset_name: str,
    ...
    # print("\nInstantiate datasets[train]!")
    # print("Sample {} for each superclass from its subclasses:".format("train_data") )
...
def _load_iirc_imagenet(root: str = ".", intask_valid_train_ratio: float = 0.04,
    ...
    train_data_path = os.path.join(root, "imageNet1K", "train")
    test_data_path = os.path.join(root, "imageNet1K", "val")
    train_data = []
    train_targets = []
    test_data = []
    test_targets = []
    ...
    # 遍历imagenet训练集的所有样本，生成 训练数据（图像路径）列表、训练目标（图像类别索引）列表
    ...
    for subclass in subclasses_to_superclass.keys():
        ...
        # samples_idx_train = samples_idx["train"]
        # samples_idx_intask_valid = samples_idx["intask_valid"]
        # samples_idx_posttask_valid = samples_idx["posttask_valid"]
        # if superclass == "electronic_equipement":
        #     print(f"superclass = \"electronic_equipement\":\nNums(_,{subclass}) : train{len(samples_idx_train)}, intask_val{len(samples_idx_intask_valid)}, posttask_val{len(samples_idx_posttask_valid)}")
...
def _get_tasks_configuration(dataset_name: str, tasks_configuration_id: int = 0, joint: bool = False) -> Tuple[
    ...
    #################################################
        ...
        assert tasks_configuration_id == 0
    #################################################
