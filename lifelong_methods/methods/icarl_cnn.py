...
class Model(BaseMethod):
    ...
    def observe(self, x: torch.Tensor, y: torch.Tensor, in_buffer: Optional[torch.Tensor] = None,
        ...
        #
        #     new_logits_new_by_pt = (torch.ones((len(output),offset_2-offset_1)) * 1e-3).cuda()
        #     pt_loss_on_new_branch = self.bce(output[:, offset_1:offset_2] / self.temperature, new_logits_new_by_pt)
        #
        #     pt_weight = 1-lambd
        #     # pt_weight = 1.0
        #     loss = cls_dist_loss + pt_weight * pt_loss_on_new_branch
        #
        #     print(f"cls_dist_loss:\n{cls_dist_loss}\n"
        #           f"{round(pt_weight, 3)} * pt_loss_on_new_branch:\n{pt_weight * pt_loss_on_new_branch}\n"
        #           f"loss:\n{loss}")
        ...
        if self.cur_task_id > 0:
            ...
            # if train:
            #     distill_loss = self.bce(output[in_buffer, :offset_1] / self.temperature, distill_model_output[in_buffer])
            # else:
            #     assert in_buffer is None
            #     distill_loss = self.bce(output[:, :offset_1] / self.temperature, distill_model_output)
            distill_loss = self.bce(output[:, :offset_1] / self.temperature, distill_model_output)
            cls_loss = self.bce(output[:, offset_1:offset_2] / self.temperature, y[:, offset_1:offset_2])

            loss = lambd * distill_loss + (1 - lambd) * cls_loss
        else:
            loss = self.bce(output / self.temperature, target)
        ...
        #     # 参考ER-ACE非对称交叉熵损失
        #     if train:
        #         if torch.all(in_buffer): # in_buffer全为True，说明该minibatch全为回放样本，new_cls_loss = 0
        #             print("in_buffer is all True, 全为回放样本 !")
        #             old_cls_loss = self.bce(output[in_buffer] / self.temperature, y[in_buffer])
        #             new_cls_loss = 0
        #         elif torch.all(~in_buffer):
        #             print("in_buffer is all False, 全为新样本 !")
        #             old_cls_loss = 0
        #             new_cls_loss = self.bce(output[:, offset_1:offset_2] / self.temperature, y[:, offset_1:offset_2])
        #         else:
        #             old_cls_loss = self.bce(output[in_buffer] / self.temperature, y[in_buffer])
        #             new_cls_loss = self.bce(output[~in_buffer, offset_1:offset_2] / self.temperature,
        #                                     y[~in_buffer, offset_1:offset_2])
        #     else:
        #         assert in_buffer is None # 只有验证集 in_buffer 为 None
        #         old_cls_loss = 0
        #         new_cls_loss = self.bce(output[:, offset_1:offset_2] / self.temperature, y[:, offset_1:offset_2])
        #
        #     # loss = distill_loss * lambd + (old_cls_loss + new_cls_loss) * (1-lambd)
        #     # loss = distill_loss * lambd + old_cls_loss + new_cls_loss * (1-lambd) # == distill_loss * lambd + (old_cls_loss * (offset_2 / (offset_2-offset_1))+ new_cls_loss) * (1-lambd)
        #     loss = distill_loss * lambd + old_cls_loss * lambd + new_cls_loss * (1 - lambd)
        # else:
        #     loss = self.bce(output / self.temperature, target)
