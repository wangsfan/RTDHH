...
from lifelong_methods.models.resnet_RTDHH import ResNet
from lifelong_methods.models.resnetcifar_RTDHH import ResNetCIFAR
from lifelong_methods.utils import get_optimizer
from lifelong_methods.buffer.buffer import BufferBase
from lifelong_methods.utils import l_distance, SubsetSampler, copy_freeze, transform_labels_names_to_vector
...
class BaseMethod(ABC, nn.Module):
    ...
    def __init__(self, n_cla_per_tsk: Union[np.ndarray, List[int]], class_names_to_idx: Dict[str, int], config: Dict):
        ...
        self.n_tasks = len(n_cla_per_tsk)
        self.class_names_to_idx = class_names_to_idx
        self.num_classes = int(sum(self.n_cla_per_tsk))
        self.cur_task_id = config["cur_task_id"]  # The current training task id
        self.temperature = config["temperature"]
        self.n_memories_per_class = config["n_memories_per_class"]
        ...
        self.aux_Psi_new_by_ft = None # 经FT得到的新类分类器，size应该是特征维度*新类数量，用于新任务开始前对新类分类器的初始化
        self.T_bt = None

        self.threshold = config["threshold"]
        self.C_norm_bdr = config["C_norm_bdr"]

        self.cur_sibling_dict = {}

        self.hier_pred_variation_mat = np.zeros((self.num_classes-self.n_cla_per_tsk[0], 2, 29))

        self.c = config['c']
        self.tp = hypnn.ToPoincare(c=self.c, train_x=False, train_c=False, ball_dim=None, riemannian=False)
        ...
        self.method_variables = ['n_cla_per_tsk', 'num_classes', 'cur_task_id', 'temperature', 'memory_strength',
                         ...
                         'lr_schedule', 'weight_decay', 'aux_Psi_new_by_ft', 'T_bt']
    ...
    # compute the prototypes only of new classes (save all features of the old classes in buffer) in hyperbolic space
    def _extract_class_means_with_memory(self, task_data: Dataset, buffer: BufferBase, low: int, high: int,
                                         device: torch.device):
        idx_to_class_names = dict((y, x) for x, y in self.class_names_to_idx.items())

        self.old_class_vecs = torch.zeros(self.n_memories_per_class, low, self.latent_dim)
        self.old_class_vecs_hyp = torch.zeros(self.n_memories_per_class, low, self.latent_dim)
        self.new_class_mean_vecs_hyp = torch.zeros(self.n_memories_per_class, high-low, self.latent_dim)
        ...
        with (torch.no_grad()):
            for class_idx in range(0, high):
                ...
                for minibatch in class_loader:
                    ...
                    if class_idx < low: # old classes
                        latent_vecs /= latent_vecs.norm(p=2, dim=-1, keepdim=True)
                        flipped_latent_vecs /= flipped_latent_vecs.norm(p=2, dim=-1, keepdim=True)
                        latent_vecs = torch.mean(torch.stack((latent_vecs, flipped_latent_vecs)), dim=0)
                    else: # new classes
                ...
                if class_idx < low:
                    self.old_class_vecs[:, class_idx, :] = latent_vectors
                else:
                    latent_vecs_hyp = self.tp(latent_vectors)
                    class_mean_hyp = pmath.poincare_mean(latent_vecs_hyp, dim=0, c=self.c)
                    self.new_class_mean_vecs_hyp[:, class_idx-low, :] = class_mean_hyp

            self.old_class_vecs_hyp = self.tp(self.old_class_vecs)
    ...
    def get_true_heirarchy_dict(self):
        ...
        # for cifar and imagenet_full, selected_supcls_to_subclses == superclass_to_subclasses;
        # for imagenet_lite/subset, the superclass_to_subclass read from ..hierarchy...josn contains the classes that the dataset without, so select
        ...
        if 'cifar' in self.dataset or 'full' in self.dataset:
            assert selected_supcls_to_subclses == superclass_to_subclasses
    ...
    def task_eval_superclass_in_training(self, Dt_train_loader:DataLoader):
        ...
        with torch.no_grad():
            ...
            # will be saved in the self.hier_pred_variation_mat
            # hier_pred_2_probs = [[0 for j in range(2)] for i in range (self.increment)]

            self.C_norm_indicator = [False] * self.increment
            self.bt_norm_indicator = [False] * offset1
            ...
            for key in super_pred_dict.keys():
                ...
                if prob_max > self.threshold:# find the superclass successfully
                ...
                if prob_max > self.C_norm_bdr:
                    self.C_norm_indicator[key_idx - offset1] = True
                if prob_max > 0.6:
                    self.bt_norm_indicator[count.argmax()] = True

                self.all_probs_pred_all[key_idx - offset1] = probs_pred_all
                self.all_keys[key_idx-offset1] = key_arr
                self.all_probs[key_idx-offset1] = prob_max


            self.C_norm_indicator = torch.tensor(self.C_norm_indicator)
            self.bt_norm_indicator = torch.tensor(self.bt_norm_indicator)
            self.all_probs_pred_all = torch.tensor(np.array(self.all_probs_pred_all))
    ...
    def prepare_model_for_new_task(self, task_data: Optional[Dataset] = None, dist_args: Optional[dict] = None,
        ...
        # if task_data.cur_task_id > self.cur_task_id:  self.cur_task_id已经在main函数中更s过了
        #     self.cur_task_id = task_data.cur_task_id

        if self.cur_task_id > 0 :
            offset1, offset2 = self._compute_offsets(self.cur_task_id)
            if self.aux_Psi_new_by_ft != None:  # initialize the new_class classifier by the aux_new_class classifier generated by FT
                if self.DP:
                    self.net.module.update_fc(offset1, offset2, self.aux_Psi_new_by_ft)
                else:
                    self.net.update_fc(offset1, offset2, self.aux_Psi_new_by_ft)
