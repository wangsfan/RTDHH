...
class ResNetCIFARModule(nn.Module):
    def __init__(self, block, num_blocks, relu_last_hidden=False):
...
class ResNetCIFAR(nn.Module):
    def __init__(self, num_classes=10, num_layers=20, relu_last_hidden=False):
        ...
        if num_layers not in [20, 32, 44, 56, 110]:
            ...
            self.convnet = ResNetCIFARModule(BasicBlock, [3, 3, 3], relu_last_hidden)
        elif num_layers == 32:
            self.convnet = ResNetCIFARModule(BasicBlock, [5, 5, 5], relu_last_hidden)
        elif num_layers == 44:
            self.convnet = ResNetCIFARModule(BasicBlock, [7, 7, 7], relu_last_hidden)
        elif num_layers == 56:
            self.convnet = ResNetCIFARModule(BasicBlock, [9, 9, 9], relu_last_hidden)
        elif num_layers == 110:
            self.convnet = ResNetCIFARModule(BasicBlock, [18, 18, 18], relu_last_hidden)

        self.fc = nn.Linear(self.convnet.latent_dim, self.num_classes)

        self.apply(_weights_init)

    # initialize the new_class classifier by the aux_new_class classifier generated by FT
    def update_fc(self, newclses_low_idx, newclses_high_idx, nextperiod_initialization):
        assert nextperiod_initialization is not None
        self.fc.weight.data[newclses_low_idx:newclses_high_idx] = nextperiod_initialization

    def forward(self, input_):
        features = self.convnet(input_)
